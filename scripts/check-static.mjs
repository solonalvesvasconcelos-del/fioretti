...
const pages=['index.html','login.html','dashboard.html','novo-chamado.html','motorista/index.html'];
let count=0;
for(const page of pages){
  ...
  for(const [,ref] of source.matchAll(/(?:src|href)="([^"]+)"/g)){
    ...
    assert.ok(fs.existsSync(path.resolve(root,path.dirname(page),ref.split('#')[0])),`${page}: ${ref}`);count++;
  }
